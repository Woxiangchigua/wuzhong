...
const query = graphql`
    query TableTODO_ApplyAllMeetingListQuery(
            $order: String = ""
            $meetingName: String = ""
				...
				,cols: [
					...
					//   ,{field:'batchNumber',width:180, title: '申请批号'}
					  ...
					  ,{field:'status', width:250, title: '会议状态', align:'center', templet : function(d){
							if(d.status === 'MEETING_END' && d.review === 'MEETING_PASS'){
								return '<span style="display:inline-block;color: #666666;border:1px solid #c9c9c9;height:22px;line-height:22px;padding:0 5px;font-size: 12px;border-radius: 2px;">已开</span>';
							}
					  }}
					  ,{fixed: 'right', title:'操作', align:'center', toolbar: '#barDemo4'}
		  			]
		  	   ]
		  ,limits: [5,10,20,50]
		});
